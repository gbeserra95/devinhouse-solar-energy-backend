	...
	public class EmailService : IEmailService
	{
		...
		public async Task SendEmailConfirmation(string email)
		{
			...
			msg.SetTemplateData(new
			{
				...
				confirmation_url = $"https://localhost:7116/api/validate?uid={user.Id}&token={token}"
			});
		...
		public async Task<PasswordForgottenResponse> ForgotPassword(string email)
		{
			...
			{
				user_name = user.FirstName,
				confirmation_url = $"https://localhost:7116/api/reset-password?uid={user.Id}&token={token}"
			});
